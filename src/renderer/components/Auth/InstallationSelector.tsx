...
export const InstallationSelector: React.FC = () => {
  const { installations, currentInstallation, selectInstallation, refreshAuth, isLoading } = useAuthStore();
  ...
  return (
    <div className="relative">
          ...
          {currentInstallation ? (
            <div className="flex items-center gap-2">
              <Avatar className="h-6 w-6">
                <AvatarImage src={currentInstallation.account.avatar_url} alt={currentInstallation.account.login} />
                <AvatarFallback>{currentInstallation.account.login[0].toUpperCase()}</AvatarFallback>
              </Avatar>
              <span className="font-medium">{currentInstallation.account.login}</span>
              <span className="text-muted-foreground text-xs">({currentInstallation.account.type})</span>
            </div>
          ) : (
      ...
      {isOpen && (
        <div className="bg-popover absolute z-10 mt-2 w-full rounded-md border p-1 shadow-md">
            ...
            >
              <div className="flex items-center gap-2">
                <Avatar className="h-6 w-6">
                  ...
                  <AvatarFallback>{installation.account.login[0].toUpperCase()}</AvatarFallback>
                </Avatar>
              ...
              {currentInstallation?.id === installation.id && <Check className="h-4 w-4" />}
            </button>
          ))}
        </div>
      )}
