...
export const TaskChat: React.FC<TaskChatProps> = ({ task: propTask }) => {
  ...
  useEffect(() => {
    const handleMessageReceived = (taskId: string, _: ChatMessage) => {
      if (taskId === task.id) {
        setIsLoading(false);
      }
    };
        ...
        {task.messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <p className="text-muted-foreground max-w-md text-sm">
              Describe what you want to build or ask questions about your code. Messages will be processed when Claude
