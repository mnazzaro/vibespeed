...
export function setupTaskHandlers(mainWindow: BrowserWindow): void {
  ...
  // Update task
  ipcMain.handle('task:update', async (event, taskId: string, updates: Partial<Task>): Promise<TaskIPCResponse<Task>> => {
    try {
  ...
  ipcMain.handle('task:sendMessage', async (
    event, 
    taskId: string, 
  ...
  ipcMain.handle('task:openInEditor', async (
    event, 
    taskId: string, 
    ...
    try {
      ...
      exec(`code "${repo.worktreePath}"`, (error) => {
        if (error) {
